...
import notifypy
import yaml
from codifica import encrypt, decrypt
import ast
class Client:
    """
    ...
    def print_msg(self, message: str) -> None:
        ...
        #         encrypted_message = ast.literal_eval(message)
        #         # Decrypt the message
        #         decrypted_message = decrypt( encrypted_message,516)
        #         print("Decrypted message:", decrypted_message)
        # except Exception as e:
        #     print("Error while decrypting message:", e)
        print(message)
            # Add the message to the stdout history
        self.stdout_history.append(message)
            # Keep the stdout history to the last 100 messages
                ...
                match received["cmd"]:
                    ...
                    case "chat":
                        ...
                        if received["nick"] in self.online_ignored_users:
                            continue
                        encrypted_message = ast.literal_eval(received["text"])
                        decrypted_message = decrypt(encrypted_message,516)

                        # Replace the 'text' field with the decrypted message
                        received["text"] = decrypted_message
